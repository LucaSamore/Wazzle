...
public final class WordCheckerImpl implements WordChecker {
    ...
    public MiniGameWord computeAttemptResult(final String attempt) {
        ...
        this.lettersLeftToGuess = targetWord.chars().mapToObj(e -> (char) e).collect(Collectors.toList());
    ...
    private void setAllCorrectLetterInTheRightPlace(final String attempt, final List<WordElement> result) {
    ...
    private void setAllRemainingLetterState(List<WordElement> result) {
       result.stream()
           ...
           .forEach(e -> e.setResult(this.lettersLeftToGuess.contains(e.getCharacter()) ?
                   Result.CORRECT_WRONG_PLACE.getState() : Result.WRONG.getState()));
    }
}
