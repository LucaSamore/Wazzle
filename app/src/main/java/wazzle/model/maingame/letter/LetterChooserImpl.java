...
public final class LetterChooserImpl implements LetterChooser {
  ...
  private final Function<Map<Character, Double>, Set<Pair<Character, Double>>> f =
      (m) ->
          m.entrySet().stream()
   ...
   *
   * @param classifiedLetters The Characters with their frequency, classified in Ranges, which have
   *     to be choosen.
   * @param gridShape The grid dimension.
   * @param mediator The mediator which handles the grid creation.
   */
  public LetterChooserImpl(
      final EnumMap<Range, WeightedAlphabet> classifiedLetters,
    ...
    Stream.of(Range.values())
        .forEach(
            r ->
                result.put(
                    r,
                    this.extractLettersFromRange(
                        this.getTotalLetterFromRange(r),
  ...
  /**
   * Get the number of letter which have to be present in a grid, proportionally to the Range
   * weight.
   *
   * @param range The Range on which the compute is done.
   * @return int which represent the number of letter of the given range have to be present in the
   *     grid.
   */
  private int getTotalLetterFromRange(final Range range) {
    double allotmentIndex =
        ((double) this.gridShape.getKey() * this.gridShape.getValue())
            / (double) Stream.of(Range.values()).map(Range::getWeight).reduce(0, (x, y) -> x + y);
   ...
   */
  private List<Pair<Character, Double>> extractLettersFromRange(
      final int numberOfExtraction, final Set<Pair<Character, Double>> pool) {
