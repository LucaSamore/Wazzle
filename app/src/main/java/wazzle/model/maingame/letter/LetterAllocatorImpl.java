...
public final class LetterAllocatorImpl implements LetterAllocator {
   ...
   *
   * @param chosenLetters The List of Character with their score ad Double which have to be
   *     allocated.
   * @param mediator The {@link Mediator} which handles the grid creation.
   */
  public LetterAllocatorImpl(
      final EnumMap<Range, List<Pair<Character, Double>>> chosenLetters, final Mediator mediator) {
  ...
  public void allocate() {
    ...
    do {
      ...
      this.choosenLetters.values().stream().forEach(c -> toAllocLetters.addAll(c));
  ...
  private int getGridSize() {
    return this.choosenLetters.values().stream().map(l -> l.size()).reduce(0, (x, y) -> x + y);
  }
  ...
  /**
   * Compute how many letter of every {@link Range} have to stay in a row/column. The number of
   * letter for {@link Range} is defined by a range Pair<Integer, Integer> - minimum and maximum
   * number of letter for range.
   *
   ...
   * @return int which represents the size of the list associated to the given {@link Range} in
   *     choosenLetters EnumMap<>
   */
   ...
   */
  //
  private Set<Letter> extractLetters(
      List<Pair<Character, Double>> toAllocLetters, Set<Pair<Integer, Integer>> grid) {
    ...
    for (var p : grid) {
      ...
      if (toAllocLetters.size() > 1) {
        ...
        allocatedLetters.add(
            new LetterImpl(
                p,
                toAllocLetters.get(extraction).getKey(),
                (int) (toAllocLetters.get(extraction).getValue() + CONSTANT_ROUND)));
        toAllocLetters.remove(extraction);
      } else {
        allocatedLetters.add(
            new LetterImpl(
                p,
                toAllocLetters.get(0).getKey(),
                (int) (toAllocLetters.get(0).getValue() + CONSTANT_ROUND)));
   ...
   * @return Set<Letter> which contains all the letter already allocated which are located in the
   *     given row.
   */
   ...
   * @return Optional<Range> which represent, if it exists, the {@link Range} which the Character
   *     belongs to.
   */
  private Optional<Range> getLetterRange(Character c) {
    Optional<Range> r = Optional.empty();
    for (var e : this.choosenLetters.entrySet()) {
      Optional<Pair<Character, Double>> pair =
          e.getValue().stream().filter(p -> p.getKey().equals(c)).findFirst();
  ...
  /**
   * Gives the number of letters which belongs to every {@link Range} based on the Letters contained
   * in the given Set.
   *
   * @param letters The set of {@link Letter} which have to be examined.
   * @return EnumMap<Range, Integer> which contains the number of {@link Letter} which belongs to
   *     every {@link Range} based on the {@link Letter} contained in the given Set
   */
  private EnumMap<Range, Integer> getAttendency(final Set<Letter> letters) {
    ...
    letters.forEach(
        l ->
            attendency.put(
                this.getLetterRange(l.getContent()).get(),
    ...
    for (var e : attendency.entrySet()) {
      result =
          Optional.of(e.getValue())
                  .filter(
                      i ->
                          i < rangeForRange.get(e.getKey()).getKey()
  ...
  private boolean checkGrid(final Set<Letter> alreadyAllocated) {
    ...
    for (int j = 0; j < this.shape; j++) {
      checkColumns =
          this.isTheLetterSetCorrect(this.getColumn(alreadyAllocated, j)) ? checkColumns : false;
    }
