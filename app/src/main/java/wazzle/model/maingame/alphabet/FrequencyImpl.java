/**
 * This class is the implementation of Frequency Interface, it counts all the letters and its number
 * of occurrences in the given Dictionary, then it returns an Object of class WeightedAlphabet with
 * a Map storing all the letters and their associated relative frequency.
 *
...
public final class FrequencyImpl implements Frequency {
  ...
  public WeightedAlphabet computeFrequency() {
    final Map<Character, Double> frequencyMap =
        this.dataset.getWords().stream()
            .flatMap(k -> k.chars().mapToObj(v -> (char) v))
            .collect(
                Collectors.groupingBy(
                    Function.identity(), Collectors.reducing(0D, e -> 1D, Double::sum)));

    final double numberOfLetters =
        frequencyMap.values().stream().mapToDouble(Double::doubleValue).sum();
