...
/**
 * This interface is an API for managing the Wazzle main game flow. It provides methods for gaining
 * information about the state of the game and operations for changing the state as well.
 */
public interface MainGame {

  /**
   * Given a word, checks if it's in {@link MainGame#wordsToBeFound()} and adds it to {@link
   * MainGame#wordsFound()}
   *
   * @return {@code true} if the given word was added to {@link MainGame#wordsFound()}, {@code
   *     false} otherwise.
   * @see java.util.Set
   */
