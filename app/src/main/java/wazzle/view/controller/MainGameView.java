...
public final class MainGameView extends View<MainGameController> {
  ...
  public void gainScoreBonus(final ActionEvent event) {
    ...
    this.titleLabel.setText(
        "Hai guadagnato "
            + (this.controller.getGame().get().getCurrentScore() - previousScore)
            + " punti!");
  }
  ...
  public void gainWordBonus(final ActionEvent event) {
    ...
    this.wordSuggestionLabel.setText(
        "Un suggerimento per te "
            ...
            + String.join(" - ", this.suggestedWords));
    ((Button) event.getSource()).setDisable(true);
  }
  ...
  protected void setGraphics() {
    ...
    this.leftPanel
        .styleProperty()
        .bind(Bindings.concat("-fx-spacing: ", this.visualUnit.asString(), ";"));
    this.rightPanel
        .styleProperty()
        .bind(Bindings.concat("-fx-spacing: ", this.visualUnit.asString(), ";"));
  ...
  private void bindElements() {
    this.visualUnit.bind(
        Bindings.min(
            stage.heightProperty().multiply(0.045), stage.widthProperty().multiply(0.045)));
    this.standardFontSize = Bindings.concat("-fx-font-size: ", this.visualUnit.asString(), ";");
    this.letterFontSize =
        Bindings.concat("-fx-font-size: ", this.visualUnit.multiply(1.5).asString(), ";");
    this.pointFontSize =
        Bindings.concat("-fx-font-size: ", this.visualUnit.multiply(0.5).asString(), ";");
    this.titleFontSize =
        Bindings.concat("-fx-font-size: ", this.visualUnit.multiply(2).asString(), ";");
  }
  ...
  private void populateGrid(final int numCols, final int numRows) {
    this.controller
        ...
        .getLetters()
        .forEach(
            l -> this.addMainGamePane(l.getPosition(), "" + l.getContent(), "" + l.getScore()));
  }

  private void addMainGamePane(
      final Pair<Integer, Integer> position, final String letter, final String points) {
    ...
    incave
        ...
        .bind(
            Bindings.min(
                this.stage.widthProperty().divide(5).multiply(0.5),
                this.stage.heightProperty().divide(5).multiply(0.5)));
    ...
    draggablePane.setOnDragDetected(
        mouseEvent -> {
          incave.startFullDrag();
          this.startReading(position, letterLabel);
        });
    draggablePane.setOnMouseDragEntered(
        mouseEvent -> this.intermediateReading(position, letterLabel));
  ...
  private void switchAfterWin() {
    try {
      ...
      SceneSwitcher.<StatisticsMainGameView>switchScene(
          this.stage, new StatisticsMainGameView(this.stage), FXMLFiles.MAIN_GAME_STATS.getPath());
    } catch (IOException e) {
      ErrorAlert.show();
    }
  }
  ...
  private boolean isNeighbour(final Pair<Integer, Integer> position) {
    return IntStream.rangeClosed(position.getKey() - 1, position.getKey() + 1)
        ...
        .flatMap(
            kk ->
                IntStream.rangeClosed(position.getValue() - 1, position.getValue() + 1)
                    .boxed()
  ...
  private void setDisableBonusButtons() {
    this.bonusScoreButton.setDisable(
        this.controller.getMainController().getBonusManager().getScoreBonusQuantity() == 0);
    this.bonusTimeButton.setDisable(
        this.controller.getMainController().getBonusManager().getTimeBonusQuantity() == 0);
    this.bonusWordButton.setDisable(
        this.controller.getMainController().getBonusManager().getWordBonusQuantity() == 0);
  }
}
