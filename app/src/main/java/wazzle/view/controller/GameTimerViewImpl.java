...
/**
 * This class is a concrete implementation for {@link GameTimer}. It provides methods for managing
 * the animation timer.
 */
public final class GameTimerViewImpl implements GameTimerView {
                ...
                new Runnable() {
                  ...
                  public void run() {
                    final var timeRemaining =
                        mainGameView.getMainGameController().getTimer().getRemainingTime();
  ...
  public void gainBonus(final ActionEvent event) {
    ...
    timerValueLabel.setText(
        "" + this.mainGameView.getMainGameController().getTimer().getRemainingTime());
  ...
  private void saveGame() throws IOException {
    this.mainGameView
        ...
        .getMainController()
        .addMainGametoHistory(this.mainGameView.getMainGameController().getGame().get());
  ...
  private void switchToMainMenu(final ActionEvent event) throws IOException {
    this.mainGameView
        .getStage()
        .setUserData(this.mainGameView.getMainGameController().getMainController());
    SceneSwitcher.<MainMenuView>switchScene(
        event, new MainMenuView(this.mainGameView.getStage()), FXMLFiles.MAIN_MENU.getPath());
  }
  ...
  private void goToMainMenu() throws IOException {
    ...
    visualUnit.bind(
        Bindings.min(
            this.mainGameView.getStage().widthProperty(),
            this.mainGameView.getStage().heightProperty()));
    MainMenuView controller = new MainMenuView(this.mainGameView.getStage());
    final Scene scene;
    scene =
        new Scene(
            Loader.<MainMenuView, Parent>loadFXMLElement(controller, FXMLFiles.MAIN_MENU.getPath()),
            this.mainGameView.getStage().getWidth(),
            this.mainGameView.getStage().getHeight());
  ...
  private void goToStatistics() throws IOException {
    ...
    scene =
        new Scene(
            Loader.<StatisticsMainGameView, Parent>loadFXMLElement(
                controller, FXMLFiles.MAIN_GAME_STATS.getPath()),
            this.mainGameView.getStage().getWidth(),
            this.mainGameView.getStage().getHeight());
    this.mainGameView.getStage().setScene(scene);
    this.mainGameView.getStage().show();
