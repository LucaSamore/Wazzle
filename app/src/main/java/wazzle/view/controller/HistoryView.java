...
/**
 * This class is a concrete implementation for {@link View}. It provides method for manage the game
 * history view.
 */
public final class HistoryView extends View<WazzleController> {
  ...
  public HistoryView(final Stage stage) {
    ...
    this.visualUnit.bind(
        Bindings.min(stage.heightProperty().multiply(0.03), stage.widthProperty().multiply(0.03)));
    this.standardFontSize = Bindings.concat("-fx-font-size: ", this.visualUnit.asString(), ";");
    this.headerFontSize =
        Bindings.concat("-fx-font-size: ", this.visualUnit.multiply(1.4).asString(), ";");
    this.titleFontSize =
        Bindings.concat("-fx-font-size: ", this.visualUnit.multiply(1.8).asString(), ";");
  }
  ...
  private void populateGrid() {
    ...
    for (MainGame mainGame : controller.getGameHistory()) {
      ...
      this.addField(
          String.valueOf(
              mainGame.getDateTime().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"))),
          indexRow,
          indexColumn,
          standardFontSize,
          paddingInsets);
      indexColumn++;
      this.addField(
          String.valueOf(mainGame.wordsFound().size()),
          indexRow,
          indexColumn,
          standardFontSize,
          paddingInsets);
      indexColumn++;
      this.addField(
          String.valueOf((int) mainGame.getCurrentScore()),
          indexRow,
          indexColumn,
          standardFontSize,
          paddingInsets);
  ...
  private void addField(
      ...
      final StringExpression fontSize,
