...
public final class SettingsView extends View<WazzleController> {
  ...
  @FXML private VBox mainSettingWindow;

  @FXML private Slider gridDimensionSlider;

  @FXML private VBox mainSettingsWrapper;

  @FXML private ComboBox<String> difficultySelectorCBox;

  @FXML private Label difficultySelectorLabel;

  @FXML private Label gridDimensionLabel;
  ...
  public SettingsView(final Stage stage) {
    ...
    this.visualUnit.bind(
        Bindings.min(stage.heightProperty().multiply(0.05), stage.widthProperty().multiply(0.05)));
  ...
  protected void buildView() {
    ...
    this.difficultySelectorCBox
        .getItems()
        .addAll(
            this.controller.getSettingsController().getAllDifficulties().stream()
    ...
    this.difficultySelectorCBox
        .getSelectionModel()
        .select(this.controller.getSettingsController().getCurrentDifficulty().getDifficultyName());
    this.gridDimensionSlider.setValue(
        this.controller.getSettingsController().getCurrentDifficulty().getGridShape());
  }

  @Override
  protected void setGraphics() {
    final ObservableValue<String> fontSize =
        Bindings.concat("-fx-font-size: ", visualUnit.asString(), ";");
    final ObservableValue<String> cbBoxItemFont =
        Bindings.concat("-fx-font-size: ", visualUnit.multiply(0.7).asString(), ";");
    final ObservableValue<String> paddingValue =
        Bindings.concat("-fx-padding: ", visualUnit.multiply(0.5).asString(), ";");
