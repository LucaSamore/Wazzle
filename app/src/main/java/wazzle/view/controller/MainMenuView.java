...
/**
 * This class is a concrete implementation for {@link View}. It provides method for manage the main
 * menu view.
 */
public final class MainMenuView extends View<WazzleController> {

  @FXML private Pane mainMenuRightPane;

  @FXML private VBox mainWrapperButtons;

  @FXML private GridPane container;

  @FXML private Button startMainGameButton;

  @FXML private Button tutorialMainGameButton;

  @FXML private Button tutorialMiniGameButton;

  @FXML private Button startMiniGameButton;
  ...
  public void nextScene(ActionEvent event) throws IOException {
    ...
    switch (node.getId()) {
      ...
      case "startMainGameButton":
        ...
        SceneSwitcher.<LoadingView>switchScene(
            event, new LoadingView(this.stage), FXMLFiles.LOADING_SCREEN.getPath());
        break;
      case "tutorialMainGameButton":
        SceneSwitcher.<TutorialMainGameView>switchScene(
            event, new TutorialMainGameView(this.stage), FXMLFiles.TUTORIAL_MAIN_GAME.getPath());
        break;
      case "tutorialMiniGameButton":
        SceneSwitcher.<TutorialMiniGameView>switchScene(
            event, new TutorialMiniGameView(this.stage), FXMLFiles.TUTORIAL_MINI_GAME.getPath());
        break;
      case "gameHistoryButton":
        this.stage.setUserData(this.controller);
        SceneSwitcher.<HistoryView>switchScene(
            event, new HistoryView(this.stage), FXMLFiles.HISTORY.getPath());
        break;
      case "settingsButton":
        this.stage.setUserData(this.controller);
        SceneSwitcher.<SettingsView>switchScene(
            event, new SettingsView(this.stage), FXMLFiles.SETTINGS.getPath());
        break;
      case "exitButton":
  ...
  protected void setGraphics() {
    this.titleFontSize =
        Bindings.concat("-fx-font-size: ", this.visualUnit.multiply(ZERO_ONE).asString(), ";");
    this.fontSize =
        Bindings.concat(
            "-fx-font-size: ", this.visualUnit.multiply(ZERO_ZERO_FIVE).asString(), ";");
    this.smallFontSize =
        Bindings.concat(
            "-fx-font-size: ", this.visualUnit.multiply(ZERO_ZERO_THREE).asString(), ";");
    ...
    this.mainWrapperButtons
        .spacingProperty()
        .bind(this.stage.heightProperty().multiply(ZERO_ZERO_FIVE));
