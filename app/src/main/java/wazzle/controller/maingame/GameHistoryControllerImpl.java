...
/**
 * This class is a concrete implementation for {@link GameHistoryController} It provides methods for
 * manage the {@link MainGame} history.
 */
public final class GameHistoryControllerImpl implements GameHistoryController {
  ...
  public MainGame getBestGame() {
    return this.gameHistory.stream()
        .filter(
            mg ->
                mg.getCurrentScore()
                    == this.gameHistory.stream()
  ...
  public List<MainGameImpl> getSortedByScoreGameHistory() {
    return this.gameHistory.stream()
        .sorted(Comparator.comparingDouble(MainGame::getCurrentScore))
